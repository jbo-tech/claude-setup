...
The block below is generated by the Penpot AI kit installer and is rewritten in place on every
install (`~/.claude/CLAUDE.md` is a symlink to this file) — never edit between its markers.

`safe-penpot-writes` is not a routing target: it adds to whichever skill the router picked, as soon
as the work writes (tokens, components, multi-page). Rule 3 below selects a domain skill, not a
substitute for the write-path discipline.
